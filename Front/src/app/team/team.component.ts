...
export class TeamComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private auth: ConnectionService, private teamService: TeamService, private route: Router, private renderer: Renderer2) {
    this.err = false;
  ...
  onSubmit(): void {
    ...
    if (this.resumeForm.valid) {
      this.teamService.createTeam(this.resumeForm.value).subscribe(
        response => {
          console.log('Form submitted with value:', response);
          localStorage.setItem("idt", response.id)
          this.route.navigate(['teamprofil']);
        },
        error => {
